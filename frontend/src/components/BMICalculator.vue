<template>
  <div class="component bmi-calc">
    <div class="container">
      <h1 class="title">BMI計算機</h1>
      <div class="container inner" id="bmi-form">
        <div class="form-item"><label class="form-label" for="height-input">身長</label><input class="form-input" type="number" v-model="height" id="height-input" name="height" placeholder="160"><span class="form-input-unit">cm</span></div>
        <div class="form-item"><label class="form-label" for="weight-input">体重</label><input class="form-input" type="number" v-model="weight" id="weight-input" name="weight" placeholder="60"><span class="form-input-unit">kg</span></div>
        <div class="form-item result">
          <span class="form-label result">BMI</span>
          <span class="form-result" v-if="validationBMI">{{ calcBMI }}</span>
          <span class="form-result error" v-else>エラー</span>
          <div class="reset-button" @click="refreshForm">
...
const validationBMI = computed(() => {
  if (!(!isNaN(weight.value) && 0 < weight.value && weight.value < 1000)) {return false;}
  if (!(!isNaN(height.value) && 0 < height.value && weight.value < 1000)) {return false;}
  return true;
});
...
const refreshForm = () => {
  height.value = "";
  weight.value = "";
  document.querySelector("#height-input").focus();
}
